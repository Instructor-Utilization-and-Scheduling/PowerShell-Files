...
function Validate-Instructor {
    ...
    If ($Instructor -in $Alias.alias){
        $alias | 
            Where-Object {$_.alias -eq $Instructor} | 
                Select-Object -ExpandProperty Name -First 1
    } # If alias
    elseif ($Instructor -in $WhiteList) {
        $Instructor #return valid instructor name
    }
    elseif ($Instructor -in "", " ") {
        $false #return false
    }
    else {
        $Instructor + " is not a valid instructor name. " + $Address |
...
function Import-ExcelSched
{
    ...
    {        
        ...
        while ($true)
        {
            ...
            $splat = @{
                OutputFilePath = $ResultsFile
                WhiteList      = $InstructorWhiteList
                alias          = $alias
            }
            ...
            foreach ($row in ($Day.Row + 1)..($NextDay.Row - 1)) {
                ...
                $Primary   = @($objWorksheet.Cells.Cells($row,  8).Text -split "[\n]|[/]|[,]" |
                    ForEach-Object {$_.Trim()} |
                        Validate-Instructor @splat | 
                            Where-Object {$_})
                $Secondary = @($objWorksheet.Cells.Cells($row, 13).Text -split "[\n]|[/]|[,]" |
                    ForEach-Object {$_.Trim()} |
                        Validate-Instructor @splat | 
                            Where-Object {$_})
    ...
    Process {
        ...
        $TotalSummary = $events | 
            Group-Object -Property Instructor |
                Select-Object @{n="sum";e={[double]($_.Group | Measure-Object -Sum duration).sum}} |
